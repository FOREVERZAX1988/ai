#!/usr/bin/env python3
"""全量标定 v4（2026-09-02）——持久磁盘输出+缺失段补采
数据源全部routes全部段; 只取 bus2(src==2) 原厂 ACC_02 idx; 视觉参照 modelV2.leadsV3[0]
留出: 每route最后2段; 分箱宽5, n>=30中位数, n<30继承42点表; <780实测+780钳制6.0+1021
"""
...
OUT='/data/openpilot/ai/tools/fit_parts'
    ...
    for m in LogReader(f):
      ...
      if w=='can':
        ...
        try:
          ...
          if len(ld)>0:
            cur_prob=float(ld[0].prob); cur_d=float(ld[0].x[0]) if len(ld[0].x)>0 else None
...
if __name__=='__main__':
  ...
  for b in sorted(bins):
    ts=bins[b]; pts[b*5+2]=statistics.median(ts) if len(ts)>=30 else old_t(b*5+2)
  ...
  json.dump({'fit_n':len(fit),'val_n':len(val),'idx':IDX,'t':[round(x,3) for x in T]}, open('/data/openpilot/ai/tools/fit_all_v4_result.json','w'))
  print("\n=== 新表代码 ===")
