...
## CAN消息 src/bus 编号权威定义（Discord 2026-08-29 用户提供）
- **0/1/2** = 在 CAN 0/1/2 上**接收**的帧
- **128/129/130** = **发送到/转发到** CAN 0/1/2 的帧（OP tx 记录；128=CAN 0）
- **192/193/194** = 尝试发送到 CAN 0/1/2 但被 **Panda 安全校验丢弃**的帧
- 分析配对：原厂帧认 src=2（雷达 CAN）、OP 代发帧认 src=128（tx→CAN 0）
- 实测（4e-seg12）：269 原厂在 src=2 全程 3001 帧（未被屏蔽）；src=1 的 2982 帧=OP 帧被 panda 转发到 CAN 1 的接收副本（与 128 逐字节相同），**不是原厂帧**；无 192-194 说明 tx 全部通过安全校验
